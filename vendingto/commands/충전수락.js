...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if (message.content.startsWith(prefix+"충전수락")) {
...
return;
            ...
            var chargecode = message.content.substring(6).split(" ")[0];
...
const mesd = new Discord.MessageEmbed() 
        			.setColor("#303136")
					 .setTitle(`**✅ㅣ${vending[userid].username}님, 충전 수락되었습니다.**`)
					 .setDescription(`금액 : ${Number(vending[userid].paymuch).toLocaleString()}원\n사유 : ` + why);
            client.users.cache.get(userid).send(mesd);
            var discordvendingmuch = Number(vending[userid].money) + Number(vending[userid].paymuch)
