...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if (message.content.startsWith(prefix+"충전취소")) {
...
return;
            ...
            var chargecode = message.content.substring(6);
            let why = "없음"
			if(message.content.substring(17).length > 0){
				why = message.content.substring(17)
			}
            if(chargecode.length !== 10) return;
            if(!charge[chargecode]) return makesimembed("해당 코드는 유효하지 않습니다.");
     var userid = charge[chargecode].buyerid;
            	if(!vending[userid]||vending[userid].state === "die") return makeembed("해당 유저는 가입되어있지 않습니다.", "", 1);
                    if(vending[userid].topay === "false"|| vending[userid].topaytype === "0") return makeembed("해당 유저는 충전 신청을 하지 않았습니다.", "", 1);
                    
          
            var oldhowmuch = vending[userid].paymuch;

const mesd = new Discord.MessageEmbed() 
        			.setColor("#303136")
            	 		.setTitle(`**❌ㅣ${vending[userid].username}님, 충전 취소되었습니다.**`)
						 .setDescription(`금액 : ${Number(vending[userid].paymuch).toLocaleString()}원\n사유 : ` + why);
            client.users.cache.get(userid).send(mesd).then(()=> {
            vending[userid] = {
            	 
                money: vending[userid].money,
                topay: "false",
                topaytype: "0",
                paymuch: "0",
                state: "live",
            ...
            fs.writeFile("./data/discordvending.json", JSON.stringify(vending, null, 2), err => {
                if (err) throw err;
                makesimembed("취소가 정상적으로 이루어졌습니다.");
