...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		message.channel.send(mesd11).then(sentMessage => {
			...
			let dbackup = fs.readFileSync('./data/discordvending.json', 'utf8');
			
				let hlbackup = fs.readFileSync('./data/list.json', 'utf8');
				
		let hbackup = fs.readFileSync('./data/hack.json', 'utf8');
		
		let rbackup = fs.readFileSync('./data/receipt.json', 'utf8');
		
		let sbackup = fs.readFileSync('./data/setting.json', 'utf8'); 
		
		let cbackup = fs.readFileSync('./data/charge.json', 'utf8'); 
		
		
		fs.writeFile("./backup/discordvending.json", dbackup, err => {
							if (err) throw err;
							done = done + 1;                  
						});
						
						fs.writeFile("./backup/list.json", hlbackup, err => {
							if (err) throw err;
							done = done + 1;                                     
						});
						
						fs.writeFile("./backup/hack.json", hbackup, err => {
							if (err) throw err;
							
							done = done + 1;                                     
						});
						fs.writeFile("./backup/receipt.json", rbackup, err => {
							if (err) throw err;
		   
							done = done + 1;                                
						});
						fs.writeFile("./backup/charge.json", cbackup, err => {
							if (err) throw err;
		 
							done = done + 1;                                
						});
		  fs.writeFile("./backup/setting.json", sbackup, err => {
						  if (err) throw err;
						  done = done + 1;               
						   ...
						   const mesd1 = new Discord.MessageEmbed() 
							.setColor("#303136")
								 .setTitle("**모든 파일이 백업이 완료되었습니다.**");
								 sentMessage.edit(mesd1)
					   }else{
					   var errdone = 6 - done
						   const mesd1 = new Discord.MessageEmbed() 
							.setColor("#303136")
								 .setTitle(errdone + "개의 파일이 백업되지 않았습니다.\n모든 파일들이 정상적으로 있는지 확인해주시기 바랍니다!");
								 sentMessage.edit(mesd1)
					   }
