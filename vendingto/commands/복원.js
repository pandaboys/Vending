...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if(message.content.startsWith(prefix+"복원")){
		...
		const mesd11 = new Discord.MessageEmbed() 
							.setColor("#303136")
								 .setTitle("**파일 복원 중..**")
		message.channel.send(mesd11).then(sentMessage => {
			
			let dbackup = fs.readFileSync('./backup/discordvending.json', 'utf8');
			
				let hlbackup = fs.readFileSync('./backup/list.json', 'utf8');
				
		let hbackup = fs.readFileSync('./backup/hack.json', 'utf8');
		
		let rbackup = fs.readFileSync('./backup/receipt.json', 'utf8');
		
		let sbackup = fs.readFileSync('./backup/setting.json', 'utf8'); 
		
		let cbackup = fs.readFileSync('./backup/charge.json', 'utf8'); 
		
		
		fs.writeFile("./data/discordvending.json", dbackup, err => {
							if (err) throw err;
		
							done = done + 1;                  
						});
						
						fs.writeFile("./data/list.json", hlbackup, err => {
							if (err) throw err;
						   
							done = done + 1;                                     
						});
						
						fs.writeFile("./data/hack.json", hbackup, err => {
							if (err) throw err;
		 
							done = done + 1;                                     
						});
						fs.writeFile("./data/receipt.json", rbackup, err => {
							if (err) throw err;
						   
							done = done + 1;                                
						});
						fs.writeFile("./data/charge.json", cbackup, err => {
							if (err) throw err;
						   
							done = done + 1;                                
						});
		  fs.writeFile("./data/setting.json", sbackup, err => {
						  if (err) throw err;
						  done = done + 1;               
						   ...
						   const mesd1 = new Discord.MessageEmbed() 
							.setColor("#303136")
								 .setTitle("**모든 파일이 복원이 완료되었습니다.**");
								 sentMessage.edit(mesd1)
					   }else{
					   var errdone = 6 - done
						   const mesd1 = new Discord.MessageEmbed() 
							.setColor("#303136")
								 .setTitle(errdone + "개의 파일이 복원되지 않았습니다.\n모든 파일들이 정상적으로 있는지 확인해주시기 바랍니다!");
								 sentMessage.edit(mesd1)
					   }
