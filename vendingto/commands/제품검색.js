...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		const nameh = message.content.substring(prefix.length + 5);
		let search = "";
		var number = 0;

		if(nameh.length <= 0) return makesimembed("제대로된 검색어를 입력해주세요!")

		hacklist["list"].list.forEach(n =>{
		...
		if(search.length <= 0){
			...
			.setColor("#303136")
			.setTitle('**'+nameh+'의 검색 결과**')
			.setDescription("검색 결과 없음")
