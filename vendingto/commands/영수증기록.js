...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if(message.content.startsWith(prefix+"영수증기록")){
...
return;
        	...
        	var receiptcodemes = message.content.substring(7);
        	if(receiptcodemes.length != 18 || !receipt[receiptcodemes]) return makesimembed("존재하지 않는 영수증 코드입니다.");
        	makesimembed("```"+receipt[receiptcodemes].description+"```영수증 기록입니다!");
        }
