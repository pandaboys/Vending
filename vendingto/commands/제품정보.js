...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if(message.content.startsWith(prefix+"제품정보")){
			if(!vending[message.author.id]||vending[message.author.id].state === "die") return makeembed("접근하실 수 없습니다.", "가입하지 않았거나 차단된 사용자입니다.", 1);
						var whathack = message.content.substring(6);
						if(!hack[whathack] || hack[whathack].deleted === "true" ) return makesimembed("존재하지 않는 상품입니다!");
						var hackamount = hack[whathack].amount;
						var hackdollar = hack[whathack].dollar;
									const sibal111 = new Discord.MessageEmbed()
								.setColor("#303136")
							.setAuthor(`${whathack} 제품의 정보`)
			.setDescription("가격 : "+hackdollar.toLocaleString()+"원\n재고 : "+hackamount+"개")
							.setTimestamp();
						message.channel.send(sibal111);
