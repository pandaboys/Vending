...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if (message.content.startsWith(prefix+"재고추가")) { 
...
return;
            ...
            var kaser = message.content.substring(6); 
            var nanne = kaser.split(" ");
            var whathack = nanne[0];
            var hackcode1 = kaser.substring(Number(whathack.length) + 1);
            var code122 = hackcode1.split(" ");
            var code10 = code122.length;
                if(!hack[whathack] || hack[whathack].deleted === "true") return makesimembed("존재하지 않는 제품입니다.");
          
          if(kaser.includes === `\n`) return makesimembed("재고를 추가하실 땐 ``"+prefix+"제고추가 (제품) (재고) (재고)`` 이 형식으로 추가하셔야합니다.");      
                
         if(hackcode1 === "" || hackcode1 === " ") return makesimembed("올바른 재고를 넣어주세요.")   ;    
		 
		 let arrayuu =  hack[whathack].code;

		 hackcode1.split(" ").forEach(cc => {
			 if(cc.length <= 0 ) return;
			 arrayuu.push(cc)
		 })
		 hack[whathack] = { 
                    code : arrayuu, amount : hack[whathack].amount + Number(code10),dollar: hack[whathack].dollar, deleted:"false"
                };
                fs.writeFile("./data/hack.json", JSON.stringify(hack, null, 2), err => {
                    if (err) throw err;
                    makesimembed("재고가 정상적으로 추가되었습니다.");
                });
