...
function makeembed(title, des, color) {
    ...
    const mesd = new Discord.MessageEmbed() 
		...
		if(message.content.startsWith(prefix+"블랙제거")){
		...
		if(vending[targetuser.id].state !== "live" ){              
							makesimembed(targetuser.username+"님을 블랙리스트에 내렸습니다.")
							client.users.cache.get(targetuser.id).send("**# 안녕하세요. 자판기봇 관리자 ("+message.guild.owner.user.username+"#"+message.guild.owner.user.discriminator+") 입니다.\n오늘부로 "+targetuser.username+"님은 블랙리스트에 [ 내려 ] 자판기봇을 이용하실 수 [ 있습니다. ] 감사합니다 #**");
						...
						/*var deleteuser0 = hacklist["userlist"].list;
						var deleteuser = deleteuser0.replace("@#~"+vending[targetuser.id].username,"");
						var deleteuserid0 = hacklist["userlist"].id;
						var deleteuserid = deleteuserid0.replace(" "+targetuser.id,"");
						 hacklist["userlist"] = { 
							  list: deleteuser, id: deleteuserid
						  };*/
						  hacklist["userlist"].list.push(targetuser.username)
						  hacklist["userlist"].id.push(targetuser.id)
